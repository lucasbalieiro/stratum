...
impl<B: IsBuffer + AeadBuffer, T: Serialize + GetSize> WithNoise<B, T> {
    ...
    fn encrypt_frame(
        ...
        if result.is_err() {
            // A failure part way through leaves the write offset at the chunk that failed, and the
            // bytes written so far in the buffer. Since the encoder is reusable, both have to be
            // undone here: otherwise the next frame is written at a stale offset with the remains
            // of this one in front of it, and the peer gets a frame it can never open.
...
mod prop_tests {
    ...
    // The glob import above brings in `crate::Result`, whose single type parameter the code
    // generated by the `Deserialize` derive cannot use.
    ...
    /// Verifies that a chunk encryption failure does not leave the encoder writing the next frame
    /// at the offset the failing chunk was using, nor with the bytes of the failed frame still in
    /// the buffer.
    #[cfg(feature = "noise_sv2")]
    #[test]
    fn noise_encoder_recovers_from_a_failed_chunk_encryption() {
        let frame = Frame::Sv2(
            Sv2Frame::<TestMessage, Slice>::from_message(TestMessage { value: 1 }, 0, 0, false)
                .unwrap(),
        );
        ...
        let next = Frame::Sv2(
            Sv2Frame::<TestMessage, Slice>::from_message(TestMessage { value: 2 }, 0, 0, false)
                .unwrap(),
        );
        let encrypted = encoder.encode(next, &mut sender_state).unwrap();

        let mut decoder = crate::StandardNoiseDecoder::<TestMessage>::new();
        let mut offset = 0;
